...
from networks.unet import UNet, UNet_DS, UNet_U2PL, UNet_URPC, UNet_CCT
import argparse
from networks.vision_transformer import SwinUnet as ViT_seg
from networks.vision_mamba import MambaUnet as ViM_seg
from networks.vision_mamba_origin import MambaUnet as ViM_seg_origin
...
def net_factory(net_type="unet", in_chns=1, class_num=4):
    if net_type == "unet":
        ...
        net = ViM_seg(config, img_size=args.patch_size,num_classes=args.num_classes).cuda()
    elif net_type == "mambaunet_origin":
        net = ViM_seg_origin(config, img_size=args.patch_size,num_classes=args.num_classes).cuda()
        # net.load_from(config)
    elif net_type == "unet_u2pl":
        net = UNet_U2PL(in_chns=in_chns, class_num=class_num).cuda()
    else:
        net = None
    return net
