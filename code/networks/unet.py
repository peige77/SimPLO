...
class UNet_U2PL(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        # Add representation head
        self.representation = nn.Sequential(
            nn.Conv2d(params['feature_chns'][-1], 256, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(256),
    ...
    def forward(self, x):
        ...
        rep = self.representation(features[-1])  # Assuming the last feature map from the encoder is used for representation
        # Upsample rep to match the input size
        rep_upsampled = F.interpolate(rep, size=(x.size(2), x.size(3)), mode='bilinear', align_corners=True)
        return {"pred": output, "rep": rep_upsampled}
